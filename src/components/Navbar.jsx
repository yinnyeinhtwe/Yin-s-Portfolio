...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-5xl px-6 md:px-10 rounded-full border border-black transition-all duration-300 ${
        scrolled
  ...
  <nav className="h-14 flex items-center justify-between">
        ...
        >
          {[0,1,2].map(i => (
            <span key={i} className={`block w-5 h-px bg-ink transition-all duration-300 ${
      ...
      <div className={`md:hidden transition-all duration-300 overflow-hidden ${
        ...
        <ul className="px-6 py-4 flex flex-col gap-3">
          {navItems.map(item => (
            <li key={item.href}>
              <a href={item.href} className="label text-ink block py-1"
