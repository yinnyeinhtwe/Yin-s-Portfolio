...
export default function Navbar() {
  ...
  return (
    <header 
      ref={navRef}
      className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-1.5rem)] max-w-5xl px-6 md:px-10 rounded-full border border-[#00AEFF] transition-all duration-300 ${
        scrolled
          ? 'bg-white/10 backdrop-blur-lg border-[#00AEFF] shadow-lg'
          : ''
      }`}
    >
    <nav className="h-14 flex items-center justify-between">
          ...
          <ul className="hidden md:flex items-center gap-9">
            {navItems.map(item => (
              <li key={item.href}>
                <a
                  href={item.href}
                  className={`nav-link text-black font-semibold text-[16px] ${active === item.href ? 'active' : ''}`}
                >{item.label}</a>
              </li>
            ))}
          </ul>
          ...
          <a
            ...
            className="hidden md:inline-flex items-center text-lg gap-1 hover:text-ink transition-colors font-semibold"
          >
            GitHub
